...
class GUI:
    # constructor method
    def __init__(self):
        logging.basicConfig(filename='peer2.log',level=logging.DEBUG, encoding='utf-8')
        ...
        try:
            if msg != "":
                ...
                if len(msg)<2048:
                    ...
                    snd= threading.Thread(target = self.sender,args=(self.channel_name,self.udp_p1_port,self.packet))
                    snd.start()
                    snd.join()
                else:
                    self.textCons.config(state = NORMAL)
